<template>
    <v-container class="pa-0">
        <v-layout wrap align-start justify-start row fill-height class="mt-0 mb-0">
            <v-flex xs6 sm3 md2 lg2 v-for="(item,i) in Speakers" :key="i" class="text-xs-center" style="text-align:center">
                ...
                <div 
                    ...
                    <v-btn class="mt-0 mx-0" icon v-if="(item.social.twitter).length>0" :href="item.twitter" target="_blank">
                        <v-icon small style="color:#1da1f2">fab fa-twitter</v-icon>
                    </v-btn>

                    <v-btn class="mt-0 mx-0" icon v-if="(item.social.linkedin).length>0" :href="item.linkedin" target="_blank">
                        <v-icon small style="color:#0077b5">fab fa-linkedin-in</v-icon>
                    </v-btn>

                    <v-btn class="mt-0 mx-0" icon v-if="(item.social.github).length>0" :href="item.github" target="_blank">
                        <v-icon small style="color:#333">fab fa-github</v-icon>
                    </v-btn>
                    <v-btn class="mt-0 mx-0" icon v-if="(item.social.website).length>0" :href="item.website" target="_blank">
...
<script>
import Speakers from '@/assets/data/speaker.json'
export default {
    data() {
        return {
            Speakers: Speakers,

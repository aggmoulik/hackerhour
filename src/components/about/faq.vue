  ...
  <v-container class="pa-0 my-0">
    <v-layout wrap align-start justify-start row fill-height class="my-0">
      <v-flex xs12 md8 lg8 class="pa-2 my-0">
        <p class="google-font" style="font-size:180%;color:#0277bd">
          FAQ
        </p>

        <v-expansion-panel>
          <v-expansion-panel-content v-for="(item, i) in faq.faq" :key="i">
            <div slot="header" class="google-font" style="font-size:120%">
              {{ item.name }}
            </div>
            <v-card>
              <v-card-text
                class="google-font grey lighten-3"
                style="font-size:110%"
                >{{ item.des }}</v-card-text
              >
            </v-card>
